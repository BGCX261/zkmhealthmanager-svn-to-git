...
public class InterceptorImcEdad5a19 extends InterceptorSelector implements
		IInterceptorGrafica {
	...
	public void mostrarGrafica(List<RespuestaInt> valores, ZKWindow zkWindow,
		...
		String mapper_grafica = sexo == ESexo.FEMENINO ? "/images/graficas/f/bmi_edad__5_19.mapper" : "/images/graficas/m/bmi_edad_5_19.mapper"; 
		final GraficaMacro graficaMacro = new GraficaMacro();
//		graficaMacro.setShowBackLine(true);
		if(sexo == ESexo.FEMENINO){
			graficaMacro.setWidthI(895);
			graficaMacro.setHeigthI(537);
			graficaMacro.setX(810d);
			graficaMacro.setY(15d);
			graficaMacro.setFocoX(65d);
			...
			graficaMacro.setWidthI(905);
			graficaMacro.setHeigthI(561);
			graficaMacro.setX(814d);
			graficaMacro.setY(13d);
			graficaMacro.setFocoX(69d);
			graficaMacro.setFocoY(507d);
			graficaMacro.setGroups(14);
			graficaMacro.setMapper(mapper_grafica);
			graficaMacro.setLengthSectX(56);
			graficaMacro.setLengthSectY(49);
		}
		...
		mostrarVentanaParaUsuario(graficaMacro, zkWindow, "Grafica de IMC para edad de 5 a 19 Anios");
	}
	...
	private List<RespuestaInt> validarLista(List<RespuestaInt> valores, PainterMoldG painterMoldG) { 
		...
		for (RespuestaInt respuestaInt : valores) {
			 ...
			 if(sexo == ESexo.FEMENINO){
				 respuestaIntSend.setY(((respuestaInt.getY() - 11) * 2)); 
			 }else{
				 respuestaIntSend.setY(((respuestaInt.getY() - 12) * 2) - 1);
			 }
			
			 respuestaIntSend.setX(((respuestaInt.getX() - 5) / 4)-9.4);
	...
	public void mostrarTabla(ESexo sexo, ZKWindow zkWindow) {
		...
		mostrarVentanaParaUsuario(tablaMacro, zkWindow, "Tabla de IMC para edad de 5 a 19 Anios");
	}
}
