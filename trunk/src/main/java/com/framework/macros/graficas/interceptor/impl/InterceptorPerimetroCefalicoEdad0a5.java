...
public class InterceptorPerimetroCefalicoEdad0a5 extends InterceptorSelector
		implements IInterceptorGrafica {

	private Timestamp fecha_nacimiento;
	private double perimetro_cefalico;
	private ESexo sexo;
	...
	public double getPerimetro_cefalico() {
		return perimetro_cefalico;
	}

	public void setPerimetro_cefalico(double perimetro_cefalico) {
	...
	public RespuestaInt getResultado() {
		if(validar()){
			...
			String tipo_tabla = ETipoTabla.getTipo(ETipoTabla.HEAD);
			...
			if(tabla_anios_meses != null){
				double perimetro_cefalico_mediana = tabla_anios_meses.getMedia();
				respuestaInt.setValor(Utilidades.calcularMedianaCurvas(perimetro_cefalico, perimetro_cefalico_mediana, tabla_anios_meses.getMenos_1_sd(), tabla_anios_meses.getMas_1_sd()));
				return respuestaInt;
			}
			throw new HealthmanagerException(" Esta no se encuentra en el rango establecido la edad: " + meses.intValue());
        }else{
       	 return null;
        }
	}
	...
	private void insertPuntosRespuesta(RespuestaInt respuestaInt){
		...
		respuestaInt.setY(perimetro_cefalico);
	}
	...
	public void mostrarGrafica(List<RespuestaInt> valores, ZKWindow zkWindow,
		...
		String mapper_grafica = sexo == ESexo.FEMENINO ? "/images/graficas/f/perimetro_celafilco_ninias_f_0_5.mapper" : "/images/graficas/m/perimetro_cefalico_0_a_5.mapper"; 
		final GraficaMacro graficaMacro = new GraficaMacro();
//		graficaMacro.setShowBackLine(true);
		if(sexo == ESexo.FEMENINO){
			graficaMacro.setWidthI(841);
			graficaMacro.setHeigthI(525);
			graficaMacro.setX(750d);
			graficaMacro.setY(22d);
			graficaMacro.setFocoX(68d);
			graficaMacro.setFocoY(474d);
			graficaMacro.setGroups(5);
			graficaMacro.setMapper(mapper_grafica);
			graficaMacro.setLengthSectX(60);
			graficaMacro.setLengthSectY(50);
		}else{
			graficaMacro.setWidthI(841);
			graficaMacro.setHeigthI(525);
			graficaMacro.setX(750d);
		...
		mostrarVentanaParaUsuario(graficaMacro, zkWindow, "Grafica de Perimetro cefalico de 0 a 5 Anios");
	}
	...
	private List<RespuestaInt> validarLista(List<RespuestaInt> valores, PainterMoldG painterMoldG) { 
		...
		for (RespuestaInt respuestaInt : valores) {
			 ...
			 if(yTemp > 29){
				 respuestaIntSend.setY(((respuestaInt.getY() - 30) * 2));
			 }
	...
	public void mostrarTabla(ESexo sexo, ZKWindow zkWindow) {
		...
		tablaMacro.setTitleValorReturn("PERIMETRO CEFALICO");
		tablaMacro.mostrarTabla(tabla_anios_meses);
		
		mostrarVentanaParaUsuario(tablaMacro, zkWindow, "Tabla de perímetro cefálico para edad de 0 a 5 Anios");
	}
}
