...
public class SuperficieAbrasion extends Convencion implements ISelector {

	@Override
	public ISectorDiente select(final SECTOR sectorSeleccionado, final int centerX,
			final int centerY, final int xTransport, final int yTransport, final int width_s,
			final int heigth_s, final int width_i, final int heigth_i, final int initX, final int initY,
		...
		return new ISectorDiente(){
			...
			public void onDraw(Graphics2D graphics2d) {
				SuperficieSectorRecita.repintarSectorSeleccionado(
						...
						graphics2d, Color.decode("#D46820")); 
			}

			@Override
			public TypeSector getTypeSector() {
				return TypeSector.ABRASION;
			}
