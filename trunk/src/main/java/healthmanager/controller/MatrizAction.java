...
public class MatrizAction extends ZKWindow implements WindowRegistrosIMG {
	...
	public void generarExcel() {
		try {
			if(validarForm()){
				...
				List<Map> resultado_matriz = getServiceLocator().getReportService()
						.getReport(map, "MatrizModel");
	...
	public boolean validarForm()throws Exception{
		...
		dtbxFechaFinal.setStyle("text-transform:uppercase;background-color:white");
		...
		if(valida && chk_entidad_eps.isChecked() && !chkAgruparPorContrato.isChecked() && getEps().isEmpty()){
			valida = false;
			msj = "Para filtar las eps debes por lo menos seleccionar una!";
		}else if(valida && chk_entidad_eps.isChecked() && chkAgruparPorContrato.isChecked() && getContratos().isEmpty()){
			valida = false;
			msj = "Para filtar por contrato debe por lo menos seleccionar uno!";
		}
		
		if(valida && chk_centro_salud.isChecked() && getCentrosSalud().isEmpty()){
			valida = false;
	...
	private void crearFilasUnidadFuncional(final Map<String, Object> bean) {
		...
		final Elemento centro_de_costo_via_ingreso = (Elemento) bean.get(SERVICIO);
		 
		row.setValue(centro_de_costo_via_ingreso);
		
		Cell cell = new Cell();
		Label label = new Label("" + centro_de_costo_via_ingreso.getDescripcion().toUpperCase());
							...
							Messagebox.QUESTION, new EventListener<Event>() {
								...
								public void onEvent(Event event) throws Exception {
									if("onYes".equals(event.getName())){
									     lista_datos_servicios.remove(bean);
	...
	public void abrirWindowServicios() {
		...
		WindowRegistrosMacro windowRegistros = new WindowRegistrosMacro(
				"Servicios", Paquetes.HEALTHMANAGER,
	...
	private void convertirAExcel(List<Map> resultado_matriz) throws Exception {
		...
		// fecha archivo
		String fecha_archivo = new SimpleDateFormat("yyyyMMddhhmmss").format(Calendar.getInstance().getTime());
...
//		styleHeader.setFont(headerFont);
		...
		celda = filaEmpresa.createCell(0,
				org.apache.poi.ss.usermodel.Cell.CELL_TYPE_STRING);
		celda.setCellValue("INFORMAcion DE PACIENTES ATENDIDOS");
		celda.setCellStyle(styleNombre_empresa);
		...
		// creamos columnas
		agregarCelda(filaCol, initCol++, "CENTRO DE ATENCION", style);
		agregarCelda(filaCol, initCol++, "TIPO ID", style);
		agregarCelda(filaCol, initCol++, "IDENTIFICAcion", style);
		agregarCelda(filaCol, initCol++, "NOMBRES", style);
		agregarCelda(filaCol, initCol++, "EDAD", style);
		agregarCelda(filaCol, initCol++, "SEXO", style);
		agregarCelda(filaCol, initCol++, "EPS", style);
		agregarCelda(filaCol, initCol++, "FECHA ATENCION", style);
		agregarCelda(filaCol, initCol++, "HORA ATENCION", style);
		agregarCelda(filaCol, initCol++, "CITA DE RIMERA VEZ", style);
		agregarCelda(filaCol, initCol++, "FECHA CITA", style);
		agregarCelda(filaCol, initCol++, "HORA CITA", style);
		agregarCelda(filaCol, initCol++, "ASISTIO A LA CITA", style);
		agregarCelda(filaCol, initCol++, "REMPLAZO", style);
		agregarCelda(filaCol, initCol++, "SERVICIOS PRESTADOS", style);
		agregarCelda(filaCol, initCol++, "DIAGNOSTICO", style);
		agregarCelda(filaCol, initCol++, "TRIAGE", style);
		agregarCelda(filaCol, initCol++, "PRESTADOR", style);
		agregarCelda(filaCol, initCol++, "VALOR ATENCION", style);
		agregarCelda(filaCol, initCol++, "REMITIDO", style);
		agregarCelda(filaCol, initCol++, "REMITIDO A", style);
		agregarCelda(filaCol, initCol++, "CENTRO DE COSTO", style);
		agregarCelda(filaCol, initCol++, "ESTUVO EN OBSERVACION", style);
		agregarCelda(filaCol, initCol++, "HORA INGRESO", style);
		agregarCelda(filaCol, initCol++, "HORA SALIDA", style);
		agregarCelda(filaCol, initCol++, "HOSPITALIZACION", style);
		agregarCelda(filaCol, initCol++, "CAMA", style);
		agregarCelda(filaCol, initCol++, "FECHA INGRESO", style);
		agregarCelda(filaCol, initCol++, "FECHA SALIDA", style);
		agregarCelda(filaCol, initCol++, "ESTADO DE SALIDA", style);
		agregarCelda(filaCol, initCol++, "FACTURADOR", style);
		...
		for (Map<String, Object> mapa : resultado_matriz) {
			...
			crearFila(mapa,
					0, // Especifica desde donde empieza la columna
					hoja1.createRow(row++),
					styleHeader);
		}
	...
	private int crearFila(Map<String, Object> mapa, int initCol, org.apache.poi.ss.usermodel.Row filaCart, CellStyle styleHeader){
		String centro_salud = (String) mapa.get("nombre_centro");
		String tipo_identificacion = (String) mapa
		...
		Timestamp fecha_atencion_correcta = fecha_atencion != null ? fecha_atencion : fecha_ingreso;
		...
		List<Map<String, Object>> list_detalle_factura = getServiceLocator().getServicio(Detalle_facturaService.class).getInformacionFactura(parametroz);
		int i = 0;
		for (Map<String, Object> map : list_detalle_factura) {
			 ...
			 servicios_prestados += codigo_servicio + " " + nombre_servicio +  ((++i < list_detalle_factura.size()) ? "\r\n" : "");
		}
		...
		String servico_cual_solicita = (String) mapa.get("servico_cual_solicita");
		String via_ingreso = (String) mapa.get("via_ingreso");  
		String observacion = (String) mapa.get("observacion"); 
		
		Timestamp fecha_ingreso_egreso = (Timestamp) mapa.get("fecha_ingreso_egreso"); 
		...
		Timestamp fecha_ingreso_hosp = (Timestamp) mapa.get("fecha_ingreso_hosp"); 
	    ...
	    String nombres_fact = (String) mapa.get("nombres_fact");
	    String apellidos_fact = (String) mapa.get("apellidos_fact");
		
		
		agregarCelda(filaCart, initCol++, edad + "", styleHeader);
		agregarCelda(filaCart, initCol++, sexo + "", styleHeader);
		agregarCelda(filaCart, initCol++, codigo_eps + " " + nombre_eps, styleHeader);
		agregarCelda(filaCart, initCol++, new SimpleDateFormat("yyyy-MM-dd").format(fecha_atencion_correcta), styleHeader); 
		agregarCelda(filaCart, initCol++, new SimpleDateFormat("hh:mm a").format(fecha_atencion_correcta), styleHeader); 
		agregarCelda(filaCart, initCol++, cita_primera_vez + "", styleHeader); 
		agregarCelda(filaCart, initCol++,  fecha_cita != null ? new SimpleDateFormat("yyyy-MM-dd").format(fecha_atencion_correcta) : "", styleHeader); 
		agregarCelda(filaCart, initCol++,  fecha_cita != null ? new SimpleDateFormat("hh:mm a").format(fecha_atencion_correcta) : "", styleHeader); 
		agregarCelda(filaCart, initCol++, estado_cita != null ? estado_cita : "", styleHeader); 
		agregarCelda(filaCart, initCol++, reemplazo + "", styleHeader); 
		agregarCelda(filaCart, initCol++, servicios_prestados + "", styleHeader); 
		agregarCelda(filaCart, initCol++, codigo_diagnostico  + " " + nombre_diagnostico, styleHeader); 
		agregarCelda(filaCart, initCol++, "" + triage, styleHeader); 
		agregarCelda(filaCart, initCol++, (nro_id_medico.equals("000000000") ? "" : (nro_id_medico  + " " + nombre_medico + " " + apellido_medico)), styleHeader); 
		agregarCelda(filaCart, initCol++, new DecimalFormat("$ #,##0.##").format(valor_total), styleHeader);  
		agregarCelda(filaCart, initCol++, "" + (remision_ext != null ? remision_ext : ""), styleHeader); 
		agregarCelda(filaCart, initCol++, servico_cual_solicita != null ? servico_cual_solicita : "", styleHeader); 
		agregarCelda(filaCart, initCol++, "" + via_ingreso, styleHeader); 
		agregarCelda(filaCart, initCol++, "" + observacion, styleHeader); 
		agregarCelda(filaCart, initCol++, fecha_ingreso_egreso != null ? new SimpleDateFormat("hh:mm a").format(fecha_ingreso_egreso) : "", styleHeader); 
		agregarCelda(filaCart, initCol++, fecha_egreso != null ? new SimpleDateFormat("hh:mm a").format(fecha_egreso) : "", styleHeader); 
		agregarCelda(filaCart, initCol++, "" + hospitalizacion, styleHeader); 
		agregarCelda(filaCart, initCol++, "" + cama, styleHeader); 
		agregarCelda(filaCart, initCol++, fecha_ingreso_hosp != null ? new SimpleDateFormat("yyyy-MM-dd hh:mm a").format(fecha_ingreso_hosp) : "", styleHeader); 
		agregarCelda(filaCart, initCol++, fecha_egreso_hosp != null ? new SimpleDateFormat("yyyy-MM-dd hh:mm a").format(fecha_egreso_hosp) : "", styleHeader); 
		agregarCelda(filaCart, initCol++, estado_salida != null ? estado_salida : "", styleHeader);
		agregarCelda(filaCart, initCol++, apellidos_fact + " " + nombres_fact, styleHeader);
