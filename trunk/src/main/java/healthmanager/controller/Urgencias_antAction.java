 ...
 *
 */
public class Urgencias_antAction  extends ZKWindow{
	
//	private static Logger log = Logger.getLogger(Urgencias_antAction.class);
	...
	@View private Label labelDestino;
	@View private Row rowFemenino0;
	...
	private void modificarPorSexo(){
		String sexo = (String)bean.get("Sexo");
		if(!sexo.isEmpty()){
			Boolean es_femenino = sexo.equalsIgnoreCase("F");
			rowFemenino0.setVisible(es_femenino);
		}
	}
	...
	private void mostrarDatos_xml()throws Exception{
		...
		if(xml!=null){
	        ...
	        for (int i = 0; i < totalRegistros; i ++){
	            ...
	            if(namedNodeMap.getNamedItem("NombreCampo").getNodeValue().equalsIgnoreCase("DestinoPaciente")){
	            	labelDestino.setValue(namedNodeMap.getNamedItem("ValorCampo").getNodeValue());
	            }
